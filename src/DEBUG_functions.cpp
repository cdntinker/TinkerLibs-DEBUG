...
void DEBUG_ESP_info()
{
    ...
    DEBUG_LineOut(Line);
    sprintf(Line, "      Flash Size: %d (%.2f MB)", FlashChipSize, FlashChipSize / 1024 / 1024);
    DEBUG_LineOut(Line);
    sprintf(Line, "     Flash speed: %u", FlashSpeed);
    DEBUG_LineOut(Line);
#if defined(ESP32)
    sprintf(Line, "      Psram Size: %d (%.2f MB)", PSramSize, PSramSize / 1024 / 1024);
    DEBUG_LineOut(Line);
#endif
    sprintf(Line, "        Ram Size: %d (%.2f MB)", RAMsize, RAMsize / 1024 / 1024);
    DEBUG_LineOut(Line);
